...
parser.add_argument('--state',
                    default=None,
                    type=int,
                    help='remain weight = 0.8**state with random pruning')
parser.add_argument('--tickets_init',
                    default=None,
                    type=str,
                    help='initilization for subnetworks')
...
def main():
    ...
    sparsity = 1 - 0.8**args.state
    print('Remaining weight (Target) = {}'.format(1 - sparsity))
    pruning_model_random(model, sparsity)
    model.cuda()
...
def save_checkpoint(state,
    ...
    if is_SA_best:
        shutil.copyfile(filepath,
                        os.path.join(save_path, 'model_SA_best.pth.tar'))
